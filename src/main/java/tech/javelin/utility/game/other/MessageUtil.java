...
public final class MessageUtil implements IMinecraft {
   ...
   public static void displayMessage(MessageUtil.LogLevel level, Object message) {
      Text icon = createGradientText("Javelin", Javelin.getInstance().getThemeManager().getCurrentTheme().getColor(), Javelin.getInstance().getThemeManager().getCurrentTheme().getSecondColor());
